# The `oklch.tools` Subpackage
The `tools` subpackage defines a whole suite of useful functions which operate on the OKLAB and OKLCH color spaces to perform tasks such as maximizing saturation, lightening and darkening, toning and detoning, etc. All of the below functions return an `OKLCH` color object unless otherwise specified.
...
## `lighten(t, color=None, hue=None, chroma=None, method='relative', use_Lr=False)`
...
- The `use_Lr` parameter uses transformed lightness `Lr` for a more gradual transition from light to dark, which is suitable for digital graphics.
