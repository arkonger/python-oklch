# The `oklch.colors` Subpackage
The `colors` subpackage defines the basic framework for color objects, as well as the conversions between them.

## The `Color` Superclass
This subpackage defines a `Color` superclass, but it should normally only be used for type-checking (e.g., `isinstance(color, Color)`).
...
`OKLAB` objects also have member functions for working with transformed lightness `Lr` as described by Ottosson in his piece on [Okhsv and Okhsl](https://bottosson.github.io/posts/colorpicker/#intermission---a-new-lightness-estimate-for-oklab):
- `get_Lr(self):` Returns the transformed lightness as a float.
- `set_Lr(self, Lr):` Returns a new OKLAB color with `a=self.a` and `b=self.b`, but with lightness equivalent to `Lr`. Do note that this is still a standard `OKLAB` object with untransformed lightness, as the setter function transforms `Lr` back to standard lightness so that all the normal operations can be applied to it without unintended consequences.
...
`OKLCH` has the additional member function `css_string(self)` which returns a string that is nicely formatted for css styles. Furthermore, `OKLCH` implements `get_Lr(self)`/`set_Lr(self, Lr)` in the same way as `OKLAB`.
